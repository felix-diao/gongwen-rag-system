...
这里的 session 指“某次妙记成功后的历史快照”，不是实时识别连接。
控制器负责历史快照的查询、查看、编辑和删除。

运维排障建议：
1. 当前纪要与历史快照不一致时，重点看 update session 日志。
2. 会话详情缺少说话人分段时，优先排查妙记成功落库阶段。
3. 删除历史记录失败时，优先核对 meeting_id 与 session_id 归属关系。
"""
...
logger = get_logger("meeting_minute_volc_session_api")

router = APIRouter(prefix="/api/meetings/minutes/volc", tags=["meeting_volc_minutes_session"])
...
def get_minutes_session(
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    if not data:
        logger.warning("获取火山纪要历史详情失败：会话不存在 meeting_id=%s session_id=%s", meeting_id, session_id)
...
def update_minutes_session(
    ...
    if not data:
        logger.warning("更新火山纪要历史会话失败：会话不存在 meeting_id=%s session_id=%s", meeting_id, session_id)
        raise HTTPException(status_code=404, detail="会话历史不存在")
...
def delete_minutes_session(
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    if not deleted:
        logger.warning("删除火山纪要历史会话失败：会话不存在 meeting_id=%s session_id=%s", meeting_id, session_id)
        raise HTTPException(status_code=404, detail="会话历史不存在")
