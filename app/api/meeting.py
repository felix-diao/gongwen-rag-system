...
# 创建会议接口
@router.post("/", response_model=schemas2.MeetingInDB)
def create_meeting(
...
# 获取当前用户创建的所有会议
@router.get("/mine", response_model=List[schemas2.MeetingInDB])
...
# 获取会议详情接口
@router.get("/{meeting_id}", response_model=schemas2.MeetingInDB)
...
# 更新会议信息接口
@router.put("/{meeting_id}", response_model=schemas2.MeetingInDB)
def update_meeting(
    meeting_id: int,
...
@router.get("/{meeting_id}/files", response_model=List[schemas2.MeetingFileInDB])
