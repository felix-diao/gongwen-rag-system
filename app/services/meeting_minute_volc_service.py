...
class VolcMeetingMinuteService:
    ...
    def update_precise_transcript(self, db: Session, meeting_id: int, text: str) -> database.MeetingAudio:
        ...
        db.query(database.VolcSpeakerSegment).filter(
            database.VolcSpeakerSegment.source_audio_id == latest_audio.id
        ).delete(synchronize_session=False)
        db.add(
            database.VolcAudioTranscription(
                meeting_id=meeting_id,
                source_audio_id=latest_audio.id,
                text=text,
                is_final=True,
            )
        )
    ...
    def upsert_summary(
        ...
        db: Session,
        meeting_id: int,
        payload: schemas.VolcMeetingSummaryCreate,
    ) -> database.VolcMeetingSummary:
        self._assert_meeting_exists(db, meeting_id)
        record = (
            db.query(database.VolcMeetingSummary)
        ...
        if record:
            ...
            record = database.VolcMeetingSummary(
                meeting_id=meeting_id,
    ...
    def create_todo(
        ...
        db: Session,
        meeting_id: int,
        payload: schemas.VolcMeetingTodoCreate,
    ) -> database.VolcMeetingTodo:
        self._assert_meeting_exists(db, meeting_id)
